app.directive('todoItemDialog', ['tasks', function (tasks) {
    return {
        restrict: 'E',
        scope: {
            todoItem: '=item',
        ...
        controller: function ($scope) {
            ...
            $scope.saveTodoItem = function () {
                tasks.add($scope.todoItem.Description, $scope.todoItem.Deadline, parseInt($scope.todoItem.ActivityId), parseInt($scope.todoItem.Duration), $scope.todoItem.Resolved);
                $scope.refreshAction();
            }
        },
        templateUrl: '/scripts/application/views/templates/todoItem.html',
        replace: true,
    }
}]);